...
| Argument | Meaning |
| --- | --- |
| `TARGET` | A URL, a connection string, or a database file, read the same way as `masume TARGET`. With none, `--profile` or `$DATABASE_URL` names the connection |
...
| `-l`, `--limit ROWS` | How many rows to return. Without it, one page of the profile |
...
The connection stands before the statement where both are written, because a run names one connection and one statement.
...
A read-only profile refuses a write in the client, before the statement reaches the server, so a job that must not write cannot write by accident.
...
`csv`, `json` and `markdown` are the formats the export form writes, so a file from a run and a file from the client read the same. A result with no rows still writes the shape of the result: `csv` writes its header and `json` writes an empty array.

A statement with no result set writes what it changed to stderr, `UPDATE 1` or `CREATE`, and nothing to stdout. The document on stdout stays the document its format says it is, so a redirect never mixes a report into the data.
...
`--explain` is the one exception: the values are written into the statement, because a planner without them estimates for the wrong values and a server does not plan a statement that still holds a placeholder.

## The plan

`--explain` writes the plan as JSON, so a check in CI reads a number out of it instead of parsing the text of a server.
...
Each node holds `depth`, `label`, `detail`, `estimatedRows`, `actualRows`, `selfMs`, `shareOfTotal`, `slowest` and `misestimated`. A count the server did not measure or estimate is `null`, never zero. The plan is measured only where the server can measure one and the statement changes nothing; `analyzed` says which of the two it was.

## Passwords

The password comes from the profile: its own value, `password_env`, or the output of `password_command`. A profile with `auth = "prompt"` cannot be run without a screen, because there is nobody to ask, and the run reports that and exits with 2.

## Several statements

A text or a file with several statements runs them in order and writes one result after another. A statement that fails stops the run, so a later one never runs on a state the failed one was supposed to make.
...
```sh
masume run -p shop 'select * from orders limit 250'    # 250 rows, because the statement says so
masume run -p shop 'select * from orders'              # 200 rows, and a note that there are more
masume run -p shop --limit 5000 'select * from orders' # 5000 rows, because the run says so
```
...
Where the page is all that came back and the result is longer, the run says so on the error stream and exits `0`:
...
The output stream holds the rows and nothing else, so a redirect still reads as the format it says it is.
...
A statement that writes is run one time and never twice, so its result is one read and not a stream. `update … returning` beyond one page writes what the read returned and reports that the rest was not read, with exit code `1`, because the output is a part and not the whole:
